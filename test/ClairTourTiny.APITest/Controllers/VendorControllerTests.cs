...
namespace ClairTourTiny.API.Tests.Controllers
{
    public class VendorControllerTests
    {
        ...
        [Fact]
        public async Task GetVendorAddresses_ReturnsInternalServerError_OnException()
        {
            // Arrange
            var vendNo = "456";
            _mockVendorService.Setup(s => s.GetVendorAddressesAsync(vendNo)).ThrowsAsync(new System.Exception());
            // Act
            var result = await _controller.GetVendorAddresses(vendNo);
            // Assert
            Assert.IsType<ObjectResult>(result.Result);
        ...
        [Fact]
        public async Task AddKnownVendor_ReturnsInternalServerError_OnException()
        {
            // Arrange
            var request = new AddKnownVendorRequestDto { PartNo = "123", VendNo = "456", SiteNo = "789" };
            _mockVendorService.Setup(s => s.AddKnownVendorAsync(request)).ThrowsAsync(new System.Exception());
            // Act
            var result = await _controller.AddKnownVendor(request);
            // Assert
            Assert.IsType<ObjectResult>(result.Result);
        ...
        [Fact]
        public async Task IsKnownVendor_ReturnsInternalServerError_OnException()
        {
            // Arrange
            var partNo = "123";
            var vendNo = "456";
            var siteNo = "789";
            _mockVendorService.Setup(s => s.IsKnownVendorAsync(partNo, vendNo, siteNo)).ThrowsAsync(new System.Exception());
            // Act
            var result = await _controller.IsKnownVendor(partNo, vendNo, siteNo);
            // Assert
            Assert.IsType<ObjectResult>(result.Result);
        ...
        [Fact]
        public async Task GetSubhireStatuses_ReturnsInternalServerError_OnException()
        {
            // Arrange
            _mockVendorService.Setup(s => s.GetSubhireStatusesAsync()).ThrowsAsync(new System.Exception());
            // Act
            var result = await _controller.GetSubhireStatuses();
            // Assert
            Assert.IsType<ObjectResult>(result.Result);
        ...
        [Fact]
        public async Task GetRateTypes_ReturnsInternalServerError_OnException()
        {
            // Arrange
            _mockVendorService.Setup(s => s.GetRateTypesAsync()).ThrowsAsync(new System.Exception());
            // Act
            var result = await _controller.GetRateTypes();
            // Assert
            Assert.IsType<ObjectResult>(result.Result);
