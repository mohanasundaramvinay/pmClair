using System;
using System.Collections.Generic;

namespace ClairTourTiny.Infrastructure.Models;

public partial class TmpEdscontractsWithOnlyProjectsImportDumbProjectRecode
{
    public string? Entityno { get; set; }

    public string? Entitydesc { get; set; }

    public DateTime? StartdateF { get; set; }

    public DateTime? EnddateF { get; set; }

    public string Custno { get; set; } = null!;

    public string Subno { get; set; } = null!;

    public string Shipno { get; set; } = null!;

    public string? Agency { get; set; }

    public string Subcontract { get; set; } = null!;

    public string InternalOrg { get; set; } = null!;

    public string Industry { get; set; } = null!;

    public string Engactivecd { get; set; } = null!;

    public string Finactivecd { get; set; } = null!;

    public int? Respempno { get; set; }

    public string Entitytype { get; set; } = null!;

    public int Prob { get; set; }

    public string Propduedate { get; set; } = null!;

    public string Proptype { get; set; } = null!;

    public string Country { get; set; } = null!;

    public string Plactivecd { get; set; } = null!;

    public string Sfactivecd { get; set; } = null!;

    public int? Opsmgr { get; set; }

    public decimal Bidmarkup { get; set; }

    public string? Locationcd { get; set; }

    public string BillingCompany { get; set; } = null!;

    public int? DefaultTaxGroup { get; set; }

    public string? Bidroot { get; set; }

    public string? Currency { get; set; }

    public DateTime ExchangeRateDate { get; set; }

    public int Readytopack { get; set; }

    public int? EngrEmpno { get; set; }

    public int? AcctExecEmpno { get; set; }

    public string? Parententityno { get; set; }

    public string? Parententitydesc { get; set; }

    public decimal? Contractiid { get; set; }

    public string? Contractid { get; set; }

    public string? Contractdescription { get; set; }

    public DateTime? Datecreated { get; set; }

    public DateTime? Startdate { get; set; }

    public DateTime? Enddate { get; set; }

    public decimal? Contactiid { get; set; }

    public decimal? Billingaddressiid { get; set; }

    public decimal? Shippingaddressiid { get; set; }

    public decimal? Type { get; set; }

    public string? Department { get; set; }

    public decimal? Periodiid { get; set; }

    public decimal? Pricingiid { get; set; }

    public decimal? Billingtermiid { get; set; }

    public decimal? Paymenttermsiid { get; set; }

    public DateTime? Nextinvoicedate { get; set; }

    public decimal? Employeeiid { get; set; }

    public decimal? Salespersoniid { get; set; }

    public decimal? Stationiid { get; set; }

    public decimal? Siteiid { get; set; }

    public decimal? Currencyiid { get; set; }

    public decimal? Totalitemsamount { get; set; }

    public decimal? Totalitemstaxamount { get; set; }

    public decimal? Totalitemsdiscount { get; set; }

    public decimal? Revision { get; set; }

    public decimal? Quoteiid { get; set; }

    public decimal? Comments { get; set; }

    public decimal? Eventiid { get; set; }

    public decimal? Evenlociid { get; set; }

    public string? Boothroomno { get; set; }

    public decimal? Ponumber { get; set; }

    public decimal? Totallaboramount { get; set; }

    public decimal? Totallabortaxamount { get; set; }

    public decimal? Totallabordiscount { get; set; }

    public decimal? Status { get; set; }

    public decimal? Deposit { get; set; }

    public decimal? Advance { get; set; }

    public decimal? Shipcarriertype { get; set; }

    public DateTime? Shipdate { get; set; }

    public decimal? Lockcount { get; set; }

    public string? Shippinglocation { get; set; }

    public decimal? Mop { get; set; }

    public decimal? Payhow { get; set; }

    public decimal? Daysinweek { get; set; }

    public string? Disclaimerid { get; set; }

    public DateTime? Revisiondate { get; set; }

    public string? Location { get; set; }

    public string? Taxcertno { get; set; }

    public DateTime? Datevalidtill { get; set; }

    public decimal? Rushcharges { get; set; }

    public decimal? Deliverycharges { get; set; }

    public decimal? Expenses { get; set; }

    public decimal? Freightcharges { get; set; }

    public decimal? Dummy1 { get; set; }

    public string? Disclaimer { get; set; }

    public decimal? Insurancepercent { get; set; }

    public decimal? Insuranceamount { get; set; }

    public decimal? Warehousestatus { get; set; }

    public decimal? Warehousedept { get; set; }

    public string? Customertaxgroup { get; set; }

    public string? Inuseby { get; set; }

    public string? Taxlabor { get; set; }

    public string? Profitcentre { get; set; }

    public decimal? Billingcustomeriid { get; set; }

    public decimal? Shippingcustomeriid { get; set; }

    public string? Commissiongroup { get; set; }

    public decimal? Billingcontactiid { get; set; }

    public string? Custdiscountgroup { get; set; }

    public decimal? Orderdiscount { get; set; }

    public decimal? Damagewaivertype { get; set; }

    public decimal? Damagewaivervalue { get; set; }

    public decimal? Defaultunit { get; set; }

    public decimal? Jobiid { get; set; }

    public decimal? Parentcontractiid { get; set; }

    public decimal? Servicechargetype { get; set; }

    public decimal? Servicechargevalue { get; set; }

    public DateTime? Lastupdated { get; set; }

    public decimal? Miscmargin { get; set; }

    public string? Uccontractdescription { get; set; }

    public DateTime? Lastprinteddate { get; set; }

    public string? Useeventpricing { get; set; }

    public decimal? Priority { get; set; }

    public string? Trackingno { get; set; }

    public string? Isworkorder { get; set; }

    public decimal? Dwtaxamount { get; set; }

    public decimal? Createdby { get; set; }

    public decimal? Probability { get; set; }

    public decimal? Servicechargetaxamount { get; set; }

    public decimal? Saleorder { get; set; }

    public decimal? Flatprice { get; set; }

    public decimal? Returncontactiid { get; set; }

    public string? Shippingcontactname { get; set; }

    public string? Returncontactname { get; set; }

    public decimal? Returncarriertype { get; set; }

    public decimal? Returnaddressiid { get; set; }

    public string? Returnboothroomno { get; set; }

    public string? Returntrackingno { get; set; }

    public string? Returnlocation { get; set; }

    public string? Istaxable { get; set; }

    public string? Periodicscheduletype { get; set; }

    public decimal? Preorposttax { get; set; }

    public string? Stopbillingflag { get; set; }

    public DateTime? Stopbillingdate { get; set; }

    public string? Project { get; set; }

    public string? Ismasterbilling { get; set; }

    public decimal? Masterbillingiid { get; set; }

    public string? Customerpricegroupid { get; set; }

    public decimal? Paytype { get; set; }

    public decimal? Returnconfirmed { get; set; }

    public decimal? Functioniid { get; set; }

    public decimal? Category { get; set; }

    public decimal? Approvalrequired { get; set; }

    public string? Paymethodreference { get; set; }

    public decimal? Damagewaiverpercent { get; set; }

    public decimal? Servicechargepercent { get; set; }

    public string? Scondiscountedamount { get; set; }

    public string? Servicechargeonsellitems { get; set; }

    public decimal? Daysinmonth { get; set; }

    public string? Viewfirstinvoice { get; set; }

    public string? Signoffstatus { get; set; }

    public string? Laborpricegroupid { get; set; }

    public string? Applytaxonservicecharge { get; set; }

    public string? Applytaxondamagewaiver { get; set; }

    public string? Shippingboothroomno { get; set; }

    public string? Defaultmainorderinfotosuborder { get; set; }

    public string? Ordercategory { get; set; }

    public string? Ordersubcategory { get; set; }

    public decimal? Consumabletype { get; set; }

    public decimal? Consumablevalue { get; set; }

    public decimal? Consumablepercent { get; set; }

    public string? Applytaxonconsumable { get; set; }

    public decimal? Cmtaxamount { get; set; }

    public decimal? Basecurrencyiid { get; set; }

    public decimal? Sitecurrencyiid { get; set; }

    public decimal? Transactioncurrencyiid { get; set; }

    public string? Roundtoid { get; set; }

    public string? Pickupbycustomer { get; set; }

    public string? Returnbycustomer { get; set; }

    public DateTime? Converteddate { get; set; }

    public decimal? Discounttype { get; set; }

    public decimal? Discountamount { get; set; }

    public string? Notificationflag { get; set; }

    public string? Isautoreceiveship { get; set; }

    public string? Isicsorder { get; set; }

    public decimal? Planneriid { get; set; }

    public string? Isforcedclosed { get; set; }

    public decimal? Totaltaxamount { get; set; }

    public string? Applyflatprice { get; set; }

    public DateTime? Lastupdateddbdate { get; set; }

    public string? Languageid { get; set; }

    public decimal? Approveddiscountperc { get; set; }

    public DateTime? Chargestartdate { get; set; }

    public DateTime? Chargeenddate { get; set; }

    public DateTime? Prepdate { get; set; }

    public DateTime? Loadindate { get; set; }

    public DateTime? Rehearsaldate { get; set; }

    public DateTime? Showstartdate { get; set; }

    public DateTime? Showenddate { get; set; }

    public DateTime? Strikedate { get; set; }

    public DateTime? Pickupdate { get; set; }

    public DateTime? Returndate { get; set; }

    public decimal? Credithold { get; set; }

    public decimal? Overheadrate { get; set; }

    public decimal? Totalorderfulldurationamt { get; set; }

    public decimal? Totalorderfulldurationtax { get; set; }

    public decimal? Firstinvoiceamount { get; set; }

    public decimal? Pricingruleiid { get; set; }

    public decimal? Accountsgroupiid { get; set; }

    public string? Readyforoperations { get; set; }

    public decimal? Burdenpercentage { get; set; }

    public string? VersionRemarks { get; set; }

    public decimal? Ordersubtype { get; set; }

    public decimal? Customstatus { get; set; }

    public decimal? Userdefinedstatus { get; set; }

    public string? ChargebasedDayweekmonthprice { get; set; }
}
