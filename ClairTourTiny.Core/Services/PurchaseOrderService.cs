...
namespace ClairTourTiny.Core.Services
{
    public class PurchaseOrderService : IPurchaseOrderService
    {
        ...
        public PurchaseOrderService(IConfiguration configuration, IMapper mapper)
        {
        ...
        public async Task<IEnumerable<PurchaseOrderModel>> GetPurchaseOrdersByProjectAsync(string entityNo)
        {
            ...
            try
            {
                ...
                var purchaseOrdersDto = new List<PurchaseOrderDto>();
                using var reader = await command.ExecuteReaderAsync();
                Console.WriteLine("Reading purchase orders from database...");
                while (await reader.ReadAsync())
                {
                    purchaseOrdersDto.Add(MapToPurchaseOrderDto(reader));
                }
                Console.WriteLine($"Successfully retrieved {purchaseOrdersDto.Count} purchase orders for project {entityNo}");
                var purchaseOrders = _mapper.Map<List<PurchaseOrderModel>>(purchaseOrdersDto);
        ...
        public async Task<IEnumerable<PurchaseOrderModel>> RefreshPurchaseOrdersAsync(string entityNo)
        {
